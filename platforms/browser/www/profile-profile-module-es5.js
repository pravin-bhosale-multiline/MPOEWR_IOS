...
var ProfilePage = /** @class */ (function () {
    function ProfilePage(loginservc, router) {
        this.loginservc = loginservc;
        this.router = router;
    }
    ProfilePage.prototype.ngOnInit = function () {
        var _this = this;
        this.loginservc.getdefaultprofile().subscribe(function (data) {
            // this.defaultprofile = data['response'];
            _this.defaultprofile = [data.response.data[0]];
            // console.log('default profile',this.defaultprofile);
            // console.log(data.response.data[0].defaultClient);
            _this.loginservc.getrolelist(_this.defaultprofile[0].id).subscribe(function (data) {
                var response = data['response'];
                _this.rolelist = response['data'];
                if (_this.defaultprofile) {
                    //set default role
                    var role = _this.rolelist.filter(function (role) { return role.role === _this.defaultprofile[0].defaultRole; })[0];
                    ...
                    var organization = { organization: _this.defaultprofile[0].defaultOrganization, organization$_identifier: _this.defaultprofile[0].defaultOrganization$_identifier };
                    _this.userorglist = [organization];
                    _this.selectedOrg = organization;
                    var warehouse = { id: _this.defaultprofile[0].defaultWarehouse, _identifier: _this.defaultprofile[0].defaultWarehouse$_identifier };
                    _this.orgwarehouselist = [warehouse];
                    _this.selectedwarehouse = warehouse;
                    //apply auto if all values available
                    if (_this.rolelist.length === 1 && organization.organization && warehouse.id) {
                        _this.onClick();
                    }
                }
    ...
    ProfilePage.prototype.onChange = function (roleValue) {
        var _this = this;
        this.selectedRole = roleValue;
        // console.log(this.selectedRole.role$_identifier);
        this.loginservc.getuserorg(this.selectedRole.userContact, this.selectedRole.mmsaSecurerule).subscribe(function (data) {
            var response = data['response'];
            // console.log("Before Sort",response);
            var tempresponse = response['data'].sort(function (a, b) { return (a._identifier < b._identifier ? -1 : 1); });
            // this.userorglist = response['data'];
            _this.userorglist = tempresponse;
            ...
            if (_this.userorglist.length === 1) {
                _this.selectedOrg = _this.userorglist[0];
                _this.onOrgChange(_this.selectedOrg);
            }
            // else{
    ...
    ProfilePage.prototype.onOrgChange = function (orgValue) {
        var _this = this;
        this.selectedOrg = orgValue;
        this.loginservc.getorgwarehouse(this.selectedOrg.organization).subscribe(function (data) {
            var response = data['response'];
            _this.orgwarehouselist = response['data'];
            _this.selectedwarehouse = _this.orgwarehouselist.filter(function (e) { return e._identifier.includes('01'); })[0];
        });
        //  console.log('selected org', this.selectedOrg.organization);
    };
    ProfilePage.prototype.onWarehouseChange = function (warehouse) {
    ...
    ProfilePage.prototype.onClick = function () {
