...
var ReportPage = /** @class */ (function () {
    function ReportPage(route, sanitizer, loginservc, router, storage, singleselservc, iab, androidPermissions, msg, nativefile, platform, fileOpener, commonFunction, alertCtrl, injector, reportService, el) {
        this.route = route;
    ...
    ReportPage.prototype.ngOnInit = function () {
        ...
        this.route.params.subscribe(function (params) {
            ...
            _this.loginservc.getReportPara(_this.rptid).subscribe(function (data) {
                _this.rptparalist = data['data'];
                _this.loginservc.reportjson['org'] = _this.loginservc.selectedprof.organization;
                _this.loginservc.reportjson['user'] = _this.loginservc.selectedprof.user;
                _this.loginservc.reportjson['role'] = _this.loginservc.selectedprof.role;
                _this.loginservc.reportjson['client'] = _this.loginservc.selectedprof.client;
                _this.loginservc.reportjson['wh'] = _this.loginservc.selectedprof.warehouse;
                _this.loginservc.reportjson['rptid'] = _this.rptid;
                for (var _i = 0, _a = _this.rptparalist; _i < _a.length; _i++) {
                    var rptpara = _a[_i];
                    //console.log(rptpara);
                    if (_this.loginservc.reportjson[rptpara.inppara] === undefined)
                        _this.loginservc.reportjson[rptpara.inppara] = rptpara[rptpara.inppara];
                    if (!_this.loginservc.iscutsomdefined && rptpara.issamepagemultiselect) {
                        var element = Object(_angular_elements__WEBPACK_IMPORTED_MODULE_15__["createCustomElement"])(_components_listcontrol_listcontrol_component__WEBPACK_IMPORTED_MODULE_16__["ListcontrolComponent"], { injector: _this.injector });
                        customElements.define('app-listcontrol', element);
                        _this.loginservc.iscutsomdefined = true;
                    }
                    if (rptpara["isview"] && rptpara["scontroltype"] === 'BTNVIEW') {
                        _this.isview = true;
                    }
                }
                _this.loginservc.GetReportPage(_this.loginservc.reportjson).subscribe(function (data) {
                    // console.log("Report Page", data);
                    _this.file_name = data['report_name'];
                    _this.txtTitle = _this.file_name.substring(0, _this.file_name.lastIndexOf("."));
                    _this.fileType = _this.file_name.substring(_this.file_name.lastIndexOf(".") + 1);
                    if (_this.fileType == 'pdf') {
                        _this.btnName = "PDF";
                    }
                    _this.innerhtml = _this.sanitizer.bypassSecurityTrustHtml(data['data']);
                    _this.loginservc.getReportPara(_this.rptid).subscribe(function (data) {
                        _this.rptparalist = data['data'];
                            ...
                            if (e) {
                                if (rptpara.isdependend) {
                                    if (rptpara.scontroltype === 'LST' && rptpara.issamepagemultiselect) {
                                        ...
                                        e.addEventListener("click", _this.onReload.bind(_this, rptpara));
                                    }
                                    else {
                                        e.addEventListener("focusin", _this.onReload.bind(_this, rptpara));
                                    }
                                }
                                else {
                                    e.addEventListener("focusin", _this.onChange.bind(_this, rptpara));
                                }
                                if (rptpara.scontroltype === 'SLS'
                                    || (rptpara.scontroltype === 'LST' && !rptpara.issamepagemultiselect)) {
                                    e.addEventListener("click", _this.onSLSClick.bind(_this, rptpara.scontroltype));
                                }
                            }
                        }
    ...
    ReportPage.prototype.onClickEmail = function () {
        var _this = this;
        this.loginservc.reportjson["inpexcel"] = 'N';
        //console.log("On email Click", this.loginservc.reportjson);
        this.loginservc.sendEmailReport(this.loginservc.reportjson).subscribe(function (data) {
            _this.response = data['response'];
            if (_this.response.messageType !== undefined) {
                if (_this.response.messageType === 'success') {
                    _this.txtmessage = _this.response.messageText;
                    _this.txterror = undefined;
                }
            }
            else {
                _this.txterror = _this.response.error.message;
                _this.txtmessage = undefined;
                _this.txterror = _this.txterror.split("Where")[0];
            }
        }, function (error) {
    ...
    ReportPage.prototype.onClickExcelPDF = function () {
        // 
        var _this = this;
        ...
        this.loginservc.downloadExcelPDF(this.loginservc.reportjson).subscribe(function (response) {
            _this.commonFunction.loadingDismiss();
            // console.log(response);
            if (response.size != 0) {
                var stype_1 = response.type;
                var blob = new Blob([response], { type: stype_1 });
                var fileName_1 = 'mydoc';
                if (_this.btnName === 'Excel') {
                    fileName_1 = _this.loginservc.selectedreport._identifier + '.csv';
                    stype_1 = 'text/csv';
                }
                else if (_this.btnName === 'PDF') {
                    fileName_1 = _this.loginservc.selectedreport._identifier + '.pdf';
                    stype_1 = 'application/pdf';
                }
                else {
                    _this.commonFunction.presentAlert("Error", "Error", "Error On Downloading Report");
                    return;
                }
                _this.commonFunction.presentAlert("Message", "Success", "Downloaded Complete.");
                //   if(!this.platform.is('cordova')){
                if (_this.msg.isplatformweb) {
                ...
                if (_this.platform.is('ios')) {
                    pathFile_1 = _this.nativefile.documentsDirectory;
                }
                if (_this.platform.is('android')) {
                    pathFile_1 = _this.nativefile.externalDataDirectory;
                }
                //  if(this.platform.is('android')|| this.platform.is('ios')){
                if (!_this.msg.isplatformweb) {
                    _this.nativefile.writeFile(pathFile_1, fileName_1, blob, {
                        replace: true,
                    }).then(function (res) {
                        //console.log('inside');
                        return _this.fileOpener.open(pathFile_1 + fileName_1, stype_1).catch(function (err) {
                            console.log(err);
                        });
                    })
                        .catch(function (error) {
                        _this.commonFunction.presentAlert("Message", "Alert", error);
                        //console.log(error);
                    });
                    ;
                }
                ...
                _this.commonFunction.presentAlert("Message", "Alert", "File not Downloaded.");
            }
        }, function (error) {
            _this.txterror = error.message;
            _this.txterror = _this.txterror.split("Where")[0];
    ...
    ReportPage.prototype.onClickView = function () {
            ...
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, this.reportService.getViewData(this.loginservc.reportjson).toPromise()];
                    case 1:
                        data = _a.sent();
                    ...
                    case 2:
                        error_1 = _a.sent();
                        this.commonFunction.loadingDismiss();
                        this.commonFunction.presentAlert("Report", "Error", error_1.error);
                        return [3 /*break*/, 3];
    ...
    //   }
    // }
    ReportPage.prototype.onReload = function (reportpara, event) {
        var _this = this;
        if (this.loginservc.reportjson[event.target.id] !== event.target.value || reportpara.scontroltype === 'LST') {
            ...
            if (event.target.value !== undefined && event.target.value !== '') {
                ...
                this.loginservc.GetReportPage(this.loginservc.reportjson).subscribe(function (data1) {
                    _this.innerhtml = _this.sanitizer.bypassSecurityTrustHtml(data1['data']);
                    _this.loginservc.getReportPara(_this.rptid).subscribe(function (data) {
                        _this.rptparalist = data['data'];
    ...
    ReportPage.prototype.onChange = function (reportpara, event) {
    ...
    ReportPage.prototype.onSLSClick = function (controltype, event) {
        // console.log(event);
    ...
    //   }
    // }
    ReportPage.prototype.onScroll = function (offsetY) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var viewHeight, limit, pageSize;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                viewHeight = this.el.nativeElement.getBoundingClientRect().height - this.headerHeight;
                if (!this.loadingIndicator && offsetY + viewHeight >= this.rows.length * this.rowHeight) {
                    limit = this.pageLimit;
                    if (this.rows.length === 0) {
                        pageSize = Math.ceil(viewHeight / this.rowHeight);
    ...
    ReportPage.prototype.loadPage = function (limit) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var results, rows;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        results = _a.sent();
                        rows = this.rows.concat(results[0].colum_data);
                        this.rows = rows;
                        this.loadingIndicator = false;
                        return [2 /*return*/];
                }
    ...
    ReportPage.prototype.presentAlert = function (Header, SubHeader, Message, entry) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            var alert;
            var _this = this;
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                    ...
                    case 0: return [4 /*yield*/, this.alertCtrl.create({
                            ...
                            buttons: [{
                                    ...
                                    handler: function (ok) {
                                        ...
                                        _this.fileOpener.open(entry.toURL(), openType)
...
var ReportService = /** @class */ (function () {
    function ReportService(http, loginauth, genericHTTP) {
        this.http = http;
        this.loginauth = loginauth;
        this.genericHTTP = genericHTTP;
    }
    ReportService.prototype.getViewData = function (body) {
